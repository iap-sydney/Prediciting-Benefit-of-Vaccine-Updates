...
# combine_data_by_immunogen = function(immunogen_data_A, immunogen_data_B, matched=TRUE, suffixes = c(".old",".new"), long_comparison_table, r, combined_immunogen_data = NULL){
...
for (r in c(1:nrow(long_comparison_table))){
  # Get the old, updated and matched data for this comparison
  this_old_immunogen_data = filter(combined_dataset_to_use,
                                   immunogen_group == long_comparison_table[r,"old_immunogen"],
                                   variant_group ==long_comparison_table[r,"future_variant"])
  this_updated_immunogen_data = filter(combined_dataset_to_use,
                                       immunogen_group == long_comparison_table[r,"updated_immunogen"],
                                       variant_group ==long_comparison_table[r,"future_variant"])
  this_matched_immunogen_data = filter(combined_dataset_to_use,
                                       immunogen_group == long_comparison_table[r,"future_variant"],
                                       variant_group ==long_comparison_table[r,"future_variant"])
  ...
  old_updated_data_paired_temp = combine_data_by_immunogen(this_old_immunogen_data, this_updated_immunogen_data,suffixes = c(".old",".new"),
                                                           long_comparison_table, r, paired=TRUE, old_updated_data_paired_temp, called_from)
  ...
  old_updated_data_all_temp = combine_data_by_immunogen(this_old_immunogen_data, this_updated_immunogen_data,suffixes = c(".old",".updated"),
                                                        long_comparison_table, r, paired=FALSE, old_updated_data_all_temp, called_from)
  ...
  updated_matched_data_paired_temp = combine_data_by_immunogen(this_updated_immunogen_data, this_matched_immunogen_data,suffixes = c(".updated",".matched"),
                                                               long_comparison_table, r, paired=TRUE, updated_matched_data_paired_temp, called_from)
  ...
  updated_matched_data_all_temp = combine_data_by_immunogen(this_updated_immunogen_data, this_matched_immunogen_data,suffixes = c(".updated",".matched"),
                                                            long_comparison_table, r, paired=FALSE, updated_matched_data_all_temp, called_from)
}
...
select_cols_join = c("paper","pairing_index","dataset", "group","another_group","prior_status", "prior_doses","old_immunogen","updated_immunogen","future_variant","comparison_name","comparison_no","variant","variant_group")
select_cols_vars = c("crhd_index","crhd_and_comparison_index","immunogen_chronology","n","fold_rise","post_neut", "prior_doses","propn_prior_infected","post_neut.ancestral", "post_neut_normalised","immunogen_group","immunogen_details","is_conflicted", "use_in_analysis")

old_updated_data_paired_long = old_updated_data_paired_temp2 %>%
...
old_updated_data_unpaired_long = old_updated_data_all_temp2 %>%
  select(any_of(c(select_cols_join,select_cols_vars))) %>%
  mutate(
    post_neut_normalised = post_neut/post_neut.ancestral,
  ...
  select(-pairing_index) %>%
  filter(!(crhd_and_comparison_index %in% old_updated_data_paired_long$crhd_and_comparison_index)) %>%
  distinct() 

old_updated_data_all_long = bind_rows(old_updated_data_paired_long,old_updated_data_unpaired_long) %>%
  distinct()

updated_matched_data_paired_long = updated_matched_data_paired_temp2 %>%
  select(any_of(c(select_cols_join,select_cols_vars))) %>%
  mutate(
    post_neut_normalised = post_neut/post_neut.ancestral,
...
updated_matched_data_unpaired_long = updated_matched_data_all_temp2 %>%
  select(any_of(c(select_cols_join,select_cols_vars))) %>%
  mutate(
    post_neut_normalised = post_neut/post_neut.ancestral,
  ...
  select(-pairing_index) %>%
  filter(!(crhd_and_comparison_index %in% updated_matched_data_paired_long$crhd_and_comparison_index)) %>%
  distinct()

updated_matched_data_all_long = bind_rows(updated_matched_data_paired_long,updated_matched_data_unpaired_long) %>%
  distinct()


all_old_updated_matched_data_long = bind_rows(old_updated_data_all_long,updated_matched_data_all_long) %>%
  ...
  left_join(old_updated_data_all_long, by = head(colnames(old_updated_data_all_long),-2)) %>%
  left_join(updated_matched_data_all_long, by = head(colnames(updated_matched_data_all_long),-2)) %>%
...
write.csv(all_old_updated_matched_data_long, file = paste0("output/all_old_updated_matched_data_long_v",version_no,"_",Sys.Date(),".csv"), row.names = F)




old_updated_paired_dataset_temp = all_old_updated_matched_data_long %>%
  filter((old_updated_has_pair), !is.na(fold_rise), immunogen_chronology != "matched_immunogen") %>%
  select(c("old_updated_pairing_index","fold_rise","post_neut","immunogen_chronology","paper","comparison_no","n"))
...
for(i in unique(old_updated_paired_dataset_temp$old_updated_pairing_index)){
  ...
  old_updated_paired_dataset_temp2 = rbind(old_updated_paired_dataset_temp2, full_join(olds, updateds, by = c("old_updated_pairing_index","paper","comparison_no"),suffix = c(".old",".updated"),relationship = "many-to-many"))
}

old_updated_paired_dataset_long = old_updated_paired_dataset_temp2 %>% 
  select(-c(immunogen_chronology.old,immunogen_chronology.updated))%>%
  mutate(old_updated_pairing_index_2 = row_number()) %>%
  melt(id.vars = c("old_updated_pairing_index","old_updated_pairing_index_2","paper","comparison_no")) %>%
  ...
  dcast(old_updated_pairing_index+old_updated_pairing_index_2+paper+comparison_no+immunogen_chronology~...,value.var="value")
